...
import { Search, ChevronLeft, Wallet, DollarSign, ArrowUpRight, History, ChevronRight, CheckCircle2, Clock, ShieldAlert, Loader2 } from "lucide-react";
...
export default function EarningsPage() {
    ...
    const fetchOrders = async () => {
        try {
            ...
            if (ordersRes.data.length > 0 && typeof window !== "undefined" && window.innerWidth >= 1280) {
    ...
    return (
        <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-[1500px] mx-auto w-full flex flex-col font-sans">
            {/* Header Section */}
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                <div>
                    <h1 className="text-2xl sm:text-[28px] font-bold text-gray-900 tracking-tight">Earnings</h1>
                    <p className="text-xs sm:text-sm font-semibold text-gray-500 mt-1.5 uppercase tracking-wide">Manage your revenue and track your payouts</p>
                </div>

                <div className="bg-white border border-gray-100 rounded-2xl p-1.5 flex gap-1 shadow-sm w-fit">
                    {filters.map((f) => (
                        <button
                            key={f}
                            onClick={() => setActiveFilter(f)}
                            className={`px-3 py-1.5 sm:px-5 sm:py-2 text-[11px] sm:text-[13px] font-bold rounded-xl transition-all ${activeFilter === f
            ...
            {!hasStripeAccount && (
                <div className="mb-8 bg-gradient-to-r from-emerald-600 to-emerald-400 rounded-3xl p-6 sm:p-8 text-white relative overflow-hidden shadow-xl shadow-emerald-100 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
                    <div className="relative z-10">
                        <h2 className="text-xl sm:text-2xl font-black tracking-tight mb-2">Complete Your Payout Setup</h2>
                        <p className="text-emerald-50 text-xs sm:text-sm font-semibold max-w-md">Connect your Stripe account to start receiving payments for your collaborations securely. It only takes a few minutes.</p>
                    </div>
                    <button
                        onClick={handleConnectStripe}
                        disabled={connectingStripe}
                        className="relative z-10 bg-white text-emerald-600 px-6 sm:px-8 py-3 rounded-2xl font-black text-xs sm:text-sm hover:bg-emerald-50 transition-all flex items-center gap-2 shadow-lg disabled:opacity-50"
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {/* Available Balance */}
                <div className="bg-white rounded-[28px] p-6 sm:p-8 shadow-sm border border-gray-100 flex flex-col justify-between relative overflow-hidden group min-h-[160px]">
                    ...
                    <div>
                        ...
                        <h2 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tighter leading-none">₹{stats.available.toLocaleString()}</h2>
                    </div>
                    <button
                ...
                <div className="bg-white rounded-[28px] p-6 sm:p-8 shadow-sm border border-gray-100 flex flex-col justify-between group h-full">
                    <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-2">TOTAL EARNED</p>
                    <div className="flex items-end justify-between mt-4">
                        <h3 className="text-2xl sm:text-3xl font-black tracking-tight text-gray-900">₹{stats.total.toLocaleString()}</h3>
                ...
                <div className="bg-white rounded-[28px] p-6 sm:p-8 shadow-sm border border-gray-100 flex flex-col justify-between group h-full">
                    ...
                    <div className="flex items-end justify-between mt-4">
                        <h3 className="text-2xl sm:text-3xl font-black tracking-tight text-blue-600">₹{stats.escrow.toLocaleString()}</h3>
                        <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-200 group-hover:text-emerald-500 transition-colors">
                            <ArrowUpRight className="w-5 h-5" />
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-[28px] p-6 sm:p-8 shadow-sm border border-gray-100 flex flex-col justify-between group h-full">
                    ...
                    <div className="flex items-end justify-between mt-4">
                        <h3 className="text-2xl sm:text-3xl font-black tracking-tight text-emerald-500">₹{stats.thisMonth.toLocaleString()}</h3>
                        <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-200 group-hover:text-emerald-500 transition-colors">
                            <ArrowUpRight className="w-5 h-5" />
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                {/* Left Side: Table */}
                <div className={`xl:col-span-2 bg-white rounded-[32px] shadow-sm border border-gray-100 flex flex-col min-h-[500px] min-w-0 ${selectedOrderId ? "hidden xl:flex" : "flex"}`}>
                    <div className="p-6 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
                        ...
                        <div className="relative w-full sm:w-60">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 bg-gray-50 border-none rounded-xl text-xs font-semibold placeholder:text-gray-400 focus:ring-2 focus:ring-emerald-500/10 transition-all"
                            />
                        </div>
                    </div>

                    {/* Desktop View: Table */}
                    <div className="hidden sm:block px-4 pb-6 overflow-auto max-h-[600px]">
                    ...
                    <div className="sm:hidden flex flex-col gap-3 p-4 max-h-[600px] overflow-y-auto">
                        {orders.map((order) => (
                            <div
                                key={order._id}
                                onClick={() => setSelectedOrderId(order._id)}
                            ...
                            >
                                <div className="flex items-center justify-between gap-3 mb-3">
                                    <span className="font-bold text-[14px] text-gray-900 truncate leading-tight min-w-0 flex-1">
                                        {order.gigId?.title || "Influencer Booking"}
                                    </span>
                                    <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest shrink-0 ${getStatusColor(order.status)}`}>
                                        {order.status}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center pt-2.5 border-t border-gray-50/60 min-w-0">
                                    <span className="text-[11px] font-semibold text-gray-400">{new Date(order.createdAt).toLocaleDateString()}</span>
                                    <span className="text-[13px] font-black text-gray-900">₹{(order.influencerAmount || 0).toLocaleString()}</span>
                                </div>
                            </div>
                        ))}
                        {orders.length === 0 && (
                ...
                <div className={`xl:col-span-1 w-full shrink-0 h-fit sticky top-24 ${selectedOrderId ? "flex" : "hidden xl:flex"}`}>
                    <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 flex flex-col w-full">
                        {selectedOrderId && selectedOrder ? (
                            <>
                                <div className="p-8 border-b border-gray-50 flex flex-col items-center relative">
                                    ...
                                    <button
                                        ...
                                        className="xl:hidden absolute top-6 left-6 flex items-center gap-1.5 text-gray-500 hover:text-emerald-600 transition-colors text-sm font-semibold"
                                    >
                            ...
                            <div className="h-full min-h-[400px] flex items-center justify-center text-gray-400 italic text-sm">
