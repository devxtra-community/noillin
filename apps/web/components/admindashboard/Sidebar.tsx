            ...
            <aside className={cn(
                ...
                <nav className="flex-1 px-4 space-y-1">
                    {filteredItems.map((item, idx) => {
                        ...
                        return (
                            <FadeIn key={idx} delay={0.1 + (idx + 1) * 0.1}>
                                <Link
                                    href={item.href}
                                    onClick={() => setIsOpen(false)}
                                    className={cn(
                                        "flex items-center gap-3 px-4 py-3 rounded-xl transition-colors duration-150 group relative",
                                        isActive
                                ...
                                >
                                    ...
                                    {isActive && (
                                        <motion.div
                                            ...
                                            className="absolute inset-0 bg-emerald-50/80 rounded-xl shadow-sm shadow-emerald-100/50"
                                            transition={{ type: "spring", stiffness: 400, damping: 30 }}
                                        />
                                    )}
                                    <item.icon size={20} className={cn("relative z-10", isActive ? "text-emerald-500" : "text-gray-400 group-hover:text-gray-900")} />
                                    <span className="text-sm relative z-10">{item.label}</span>
                                    {item.badge && (
                                        <span className="ml-auto bg-orange-100 text-orange-600 text-[10px] font-bold px-2 py-0.5 rounded-full relative z-10">
                                            {item.badge}
                                        </span>
                                    )}
                                    {/* Animated right indicator pill */}
                                    {isActive && (
                                        <motion.div
                                            layoutId="sidebar-active-pill"
                                            className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-emerald-500 rounded-l-full z-10"
                                            transition={{ type: "spring", stiffness: 400, damping: 30 }}
                                        />
                                    )}
                                </Link>
                ...
                {/* Footer Section */}
                <FadeIn delay={0.1 + (filteredItems.length + 1) * 0.1}>
